...
# Compatibility
SwapTube is developed, and is known to compile and run on several Linux distributions. Windows is untested.

There are three compute backends, selected automatically by CMake:

| Backend | Requires | Notes |
|---------|----------|-------|
| CUDA | A CUDA-compatible NVIDIA GPU | The reference backend |
| HIP | A [HIP](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)-compatible AMD GPU | |
| CPU | Nothing | Used when neither GPU toolkit is found. Runs everywhere, including macOS on Apple silicon |

On macOS the CPU backend is additionally accelerated by Metal for the kernels listed in `src/Metal/ported.manifest`; see [macOS](#macos) below.

Note that the HIP and CPU folders are generated by translating the CUDA folder — with hipify and `cmake/translate_cuda_to_cpu.py` respectively. Do not modify any of the contents in `HIP/` or `CPU/`; treat the CUDA folder as the source of truth and modify that. They will be re-translated via CMake.

## macOS
macOS builds run on the CPU backend and encode through VideoToolbox, so Apple's
media engine still does the HEVC encoding in hardware. Install the dependencies
and go:

```bash
brew install cmake ninja pkg-config ffmpeg librsvg glib cairo libpng gnuplot mpv \
    cairomm pangomm tinyxml2 fontconfig
./go.sh MandelbrotDemo 640 360 30
```

The first `./go.sh` offers to build MicroTeX for you. Accept: on macOS it runs
`cmake/setup_microtex_macos.sh`, which builds MicroTeX's headless LaTeX-to-SVG
converter. Upstream's Unix target needs `gtksourceviewmm-3.0`, which is
deprecated and has no Homebrew formula, and SwapTube only ever invokes the
headless path anyway.

### Metal acceleration
The kernels listed in `src/Metal/ported.manifest` run on the GPU through Metal;
everything else stays on the CPU backend. Both share the same buffers, since
device allocations are `MTLBuffer`s with shared storage and Apple silicon has one
physical memory pool, so a frame can mix the two with no copying.

Measured on an M4 at 640x360, whole-render wall time:
...
path needs nothing beyond the Command Line Tools.

What to expect without Metal: light scenes are comfortable and heavy raymarching
is not. On an M4 (10 cores) at 1280x720, `LatexDemo` renders at roughly 54 fps
while `MandelbulbDemo` takes about 2 seconds per frame. Block dispatch scales
close to linearly with core count. `SWAPTUBE_THREADS` overrides the worker count
if you want to leave cores free, and `SWAPTUBE_PROFILE_KERNELS=1` prints a
per-kernel time breakdown at exit, which is how the porting order was chosen.

### Porting another kernel to Metal
1. Add the `src/CUDA` file to `src/Metal/ported.manifest`. That suffixes its
   `extern "C"` entry points with `_cpu` so both implementations link, and
   generates an argument struct plus an unpack macro from the CUDA signature.
2. Write `src/Metal/<name>.metal`. Include `MetalPrelude.h`, then
   `generated/<name>_device.h` for the file's `__device__` helpers, then
   `generated/kernel_args.h`. The kernel body can usually be the CUDA body
   verbatim: the unpack macro re-creates the parameters as locals, so only the
   entry signature, the thread indexing and the pixel writes change.
3. Write `src/Metal/<name>.cpp`, the host wrapper that fills the argument struct
   and dispatches, falling back to the `_cpu` entry point.
4. Record the kernel body hash the generator prints. `--check` compares it later,
   so an upstream change to a ported CUDA kernel is reported as drift rather than
   silently diverging.
...
| CUDA or HIP/ROCm | Computationally intensive graphics | Video render loop | Various | Hardware-dependent. Optional: without either, CMake builds the CPU backend instead |
