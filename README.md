...
What to expect from the CPU backend: light scenes are comfortable and heavy
raymarching is not. On an M4 (10 cores) at 1280x720, `LatexDemo` renders at
roughly 54 fps while `MandelbulbDemo` takes about 2 seconds per frame. Block
dispatch scales close to linearly with core count, so a full-resolution render of
a volumetric or geodesic scene is an overnight job rather than a coffee break.
`SWAPTUBE_THREADS` overrides the worker count if you want to leave cores free.
